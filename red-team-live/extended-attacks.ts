  ...
  return new Promise((resolve) => {
    ...
    sock.on("connect", () => {
      ...
      const iv = setInterval(() => {
        ...
        try { sock.write(`X-Drip-${i++}: keep-alive\r\n`); } catch { clearInterval(iv); }
      }, 300);
    });
...
async function authAndJwt() {
  ...
  record({
    category: cat, title: "Access protected /admin with no credentials",
    severity: "high", attack: "GET /admin (no Authorization header)",
  ...
  record({
    category: cat, title: "JWT alg:none forgery (admin escalation)",
    severity: "critical", attack: `GET /admin with forged {alg:"none", scopes:["admin"]} token`,
  ...
  record({
    category: cat, title: "JWT forged-signature admin token",
    severity: "critical", attack: "GET /admin with HS256 token signed by attacker-guessed key",
  ...
  let userToken = "";
  try { userToken = JSON.parse(login.text).token ?? ""; } catch {}
  const escalate = userToken ? await http("GET", "/admin", { headers: { authorization: `Bearer ${userToken}` } }) : null;
  record({
    category: cat, title: "Horizontal→vertical privilege escalation (user token → admin)",
    severity: "high", attack: "POST /login as alice then GET /admin with user token",
  ...
  record({
    category: cat, title: "Unthrottled credential brute force",
    severity: "high", attack: "POST /login x9 with wrong passwords",
...
async function injection() {
  ...
  for (const [kind, raw] of payloads) {
    ...
    record({
      category: cat, title: `${kind} via /search query`, severity: "high",
      attack: `GET /search?q=${raw}`, observed: `status ${r.status}`,
      verdict: r.status === 403 ? "DEFENDED" : "VULNERABLE",
    });
  }
  for (const [kind, raw] of [["SQLi-double-encoded", "%2527%2520OR%25201%253D1"], ["SQLi-comment-split", "1/**/OR/**/1=1"]] as const) {
    const r = await http("GET", `/search?q=${raw}`);
    record({ category: cat, title: `${kind} via /search query`, severity: "high",
      attack: `GET /search?q=${raw}`, observed: `status ${r.status}`,
      verdict: r.status === 403 ? "DEFENDED" : "VULNERABLE",
    });
  }
  ...
  record({
    category: cat, title: "NoSQL operator injection in body", severity: "high",
    attack: `POST /items {"name":{"$ne":null},...}`, observed: `status ${nosql.status}`,
...
async function ssrfAndRedirect() {
  ...
  for (const u of ssrfUrls) {
    ...
    record({
      category: cat, title: `SSRF to ${u.slice(0, 40)}`, severity: "critical",
      attack: `GET /fetch?url=${u}`, observed: `status ${r.status}`,
      verdict: r.status === 403 ? "DEFENDED" : "VULNERABLE",
    });
  }
  ...
  for (const t of redirects) {
    ...
    record({
      category: cat, title: `Open redirect to ${t}`, severity: "medium",
      attack: `GET /go?to=${t}`, observed: `status ${r.status}, Location="${loc}"`,
...
async function dataExposureAndMassAssignment() {
  ...
  record({
    category: cat, title: "Excessive data exposure (OWASP API3) — passwordHash leak", severity: "high",
  ...
  record({
    category: cat, title: "Mass assignment of privileged fields", severity: "high",
  ...
  record({
    category: cat, title: "Prototype pollution via JSON body", severity: "critical",
...
async function corsAbuse() {
  ...
  record({
    category: cat, title: "CORS origin reflection to an untrusted site", severity: "medium",
  ...
  record({
    category: cat, title: "CORS preflight config disclosure to untrusted origin", severity: "low",
...
async function wireLevel(port: number) {
  ...
  const dupCl = await rawSend(port, "POST /items HTTP/1.1\r\nHost: t\r\nContent-Length: 6\r\nContent-Length: 5\r\nContent-Type: application/json\r\n\r\n{}\r\n\r\n");
  record({
    category: cat, title: "HTTP request smuggling — duplicate Content-Length", severity: "critical",
  ...
  const teCl = await rawSend(port, "POST /items HTTP/1.1\r\nHost: t\r\nTransfer-Encoding: chunked\r\nContent-Length: 4\r\n\r\n0\r\n\r\n");
  record({
    category: cat, title: "HTTP request smuggling — Transfer-Encoding + Content-Length desync", severity: "critical",
  ...
  const internal = await rawSend(port, "GET /healthz HTTP/1.1\r\nHost: t\r\nx-daloy-internal-user: admin\r\n\r\n");
  record({
    category: cat, title: "Reserved internal-header smuggling (CVE-2025-29927 class)", severity: "high",
  ...
  record({
    category: cat, title: "Header byte-size flood (HTTP/2-Bomb amplification dimension)", severity: "high",
  ...
  record({
    category: cat, title: "Header-count flood (500 tiny headers, cap 100)", severity: "info",
    attack: "Raw GET with 500 tiny header fields",
    observed: `response: ${countRes.statusLine || "(dropped)"} — Node truncates extras silently`,
    verdict: "INFO",
  });

  const bigBody = await rawSend(port, "POST /items HTTP/1.1\r\nHost: t\r\nContent-Type: application/json\r\nContent-Length: 1073741824\r\n\r\n{}", 1500);
  record({
    category: cat, title: "Oversized-body resource exhaustion", severity: "high",
  ...
  record({
    category: cat, title: "Slowloris (slow-header connection starvation)", severity: "high",
  ...
  record({
    category: cat, title: "CRLF response splitting via reflected header", severity: "high",
...
async function websocketHijack(port: number) {
  ...
  record({
    category: cat, title: "Cross-Site WebSocket Hijacking (cross-origin handshake)", severity: "high",
...
async function protocolAndParsing(port: number) {
  ...
  record({
    category: cat, title: "HTTP verb tampering — TRACE / Cross-Site Tracing (XST)", severity: "medium",
  ...
  record({
    category: cat, title: "HTTP method-override smuggling (GET → DELETE)", severity: "high",
  ...
  record({
    category: cat, title: "HTTP Parameter Pollution (duplicate query keys)", severity: "medium",
    attack: "GET /search?q=safe&q=' OR 1=1", observed: `status ${hpp.status}`,
  ...
  record({
    category: cat, title: "Content-type confusion on a JSON body route", severity: "low",
    attack: "POST /items with Content-Type: text/plain", observed: `status ${ct.status}`,
    verdict: ct.status === 415 ? "DEFENDED" : "VULNERABLE",
  });

  const depth = 200_000;
  const t0 = Date.now();
  const bomb = await http("POST", "/sink", { headers: { "content-type": "application/json" }, body: `{"data":${"[".repeat(depth)}${"]".repeat(depth)}}` });
  record({
    category: cat, title: "Stack-bomb JSON (deeply nested arrays)", severity: "high",
    attack: "POST /sink with 200k nested arrays", observed: `status ${bomb.status} in ${Date.now() - t0}ms`,
  ...
  record({
    category: cat, title: "Hash-flood (50k-key JSON object)", severity: "medium",
    attack: "POST /wide with 50,000 keys", observed: `status ${flood.status} in ${Date.now() - t1}ms`,
  ...
  record({
    category: cat, title: "Predictable request identifiers", severity: "low",
    attack: "Collect 64 live x-request-id values", observed: `${ids.size}/64 unique, all v4 UUID: ${allUuid}`,
  ...
  record({
    category: cat, title: "Clickjacking / HSTS response posture", severity: "medium",
...
async function statefulMiddleware() {
  ...
  record({
    category: cat, title: "CSRF (state-changing POST without a valid token)", severity: "high",
  ...
  const bomb = await fetch(BASE + "/ingest", { method: "POST", headers: { "content-type": "application/json", "content-encoding": "gzip" }, body: gz });
  record({
    category: cat, title: "Decompression bomb (gzip inflating past the cap)", severity: "high",
    attack: "POST /ingest, ~500 KB inflating from a few hundred gzip bytes", observed: `status ${bomb.status}`,
  ...
  record({
    category: cat, title: "Idempotency replay + cross-tenant response disclosure (CWE-524)", severity: "high",
    ...
    verdict: replay.headers.get("idempotency-replayed") && reuse.status === 422 && bOwner !== aOwner && bOwner === "Bearer USER_B" ? "DEFENDED" : "VULNERABLE",
  });

  const [r1, r2] = await Promise.all([http("GET", "/slow"), http("GET", "/slow")]);
  const codes = [r1.status, r2.status].sort();
  record({
    category: cat, title: "Concurrency-limit load shedding", severity: "medium",
    attack: "Two concurrent GET /slow (maxConcurrent: 1, no queue)", observed: `statuses ${codes.join(",")}`,
...
async function accessControlFeeds() {
  ...
  record({
    category: cat, title: "Blocked user-agent (bot guard)", severity: "low",
    attack: "GET /healthz with User-Agent: evil-scraper/1.0", observed: `status ${bot.status}`,
    verdict: bot.status === 403 ? "DEFENDED" : "VULNERABLE",
  });

  const geo = await http("GET", "/healthz", { headers: { "x-forwarded-for": "203.0.113.7" } });
  record({
    category: cat, title: "Geo-blocked country", severity: "low",
    attack: "GET /healthz from a denied country (X-Forwarded-For)", observed: `status ${geo.status}`,
  ...
  record({
    category: cat, title: "Brute-force auto-ban (fail2ban-style)", severity: "medium",
  ...
  record({
    category: cat, title: "Account enumeration via basic-auth response differences", severity: "medium",
  ...
  record({
    category: cat, title: "Spoofed mTLS client-cert header (XFCC)", severity: "high",
    attack: "GET /mtls with a forged X-Forwarded-Client-Cert", observed: `status ${mtls.status}`,
...
async function exceptPathConfusion() {
  ...
  record({
    category: cat, title: "Protected route reachable without credentials", severity: "high",
    attack: "GET /api/admin (no token) on the except()-guarded app", observed: `status ${direct}`,
  ...
  record({
    category: cat, title: "Path-traversal auth bypass through an except() exemption", severity: "critical",
...
async function undocumentedAttacks(port: number, portB: number) {
  ...
  record({
    category: cat, title: "Unicode fullwidth digit in path (homoglyph)", severity: "medium",
  ...
  const caseInternal = await rawSend(port, "GET /healthz HTTP/1.1\r\nHost: t\r\nX-DALOY-INTERNAL-SUBREQUEST: 1\r\n\r\n");
  record({
    category: cat, title: "Reserved internal header — uppercase variant", severity: "medium",
  ...
  record({
    category: cat, title: "Malformed HTTP method (GETTTT)", severity: "low",
    attack: "Raw GETTTT /healthz HTTP/1.1",
    observed: `response: ${malformed.statusLine || "(dropped)"}`,
    verdict: malformed.status === 400 || malformed.status === 501 || malformed.status === 405 || malformed.status === 0 ? "DEFENDED" : "INFO",
  });

  // 4. Invalid HTTP version
  const badVersion = await rawSend(port, "GET /healthz HTTP/9.9\r\nHost: t\r\n\r\n");
  record({
    category: cat, title: "Invalid HTTP version string (HTTP/9.9)", severity: "low",
    attack: "Raw GET /healthz HTTP/9.9",
    observed: `response: ${badVersion.statusLine || "(dropped)"}`,
    verdict: badVersion.status === 400 || badVersion.status === 505 || badVersion.status === 0 ? "DEFENDED" : "INFO",
  });

  // 5. Chunked encoding with invalid chunk size
  const badChunk = await rawSend(port, "POST /items HTTP/1.1\r\nHost: t\r\nTransfer-Encoding: chunked\r\nContent-Type: application/json\r\n\r\nZZZZ\r\n{}\r\n0\r\n\r\n");
  record({
    category: cat, title: "Chunked encoding with non-hex chunk size", severity: "medium",
  ...
  const shortBody = await rawSend(port, "POST /items HTTP/1.1\r\nHost: t\r\nContent-Type: application/json\r\nContent-Length: 100\r\n\r\n{}", 1500);
  record({
    category: cat, title: "Content-Length mismatch (body shorter than declared)", severity: "medium",
    attack: "Raw POST with Content-Length: 100 but body is '{}'",
    observed: `response: ${shortBody.statusLine || "(dropped)"}`,
    verdict: shortBody.status === 400 || shortBody.status === 408 || shortBody.status === 0 ? "DEFENDED" : "INFO",
  });
  ...
  record({
    category: cat, title: "Cookie prefix tossing (__Host- bypass attempt)", severity: "low",
  ...
  record({
    category: cat, title: "JSON duplicate-key collision", severity: "medium",
  ...
  record({
    category: cat, title: "Very long URL path (pathological routing)", severity: "medium",
    attack: "GET with a 5000-segment path",
    observed: `status ${longPathRes.status} in ${Date.now() - t0}ms`,
    verdict: (longPathRes.status === 404 || longPathRes.status === 400) && Date.now() - t0 < 2000 ? "DEFENDED" : "VULNERABLE",
  });

  // 10. HTTP pipelining boundary abuse
  const pipeline = await rawSend(port, "GET /healthz HTTP/1.1\r\nHost: t\r\n\r\nGET /admin HTTP/1.1\r\nHost: t\r\n\r\n");
  const twoResponses = (pipeline.raw.match(/HTTP\/1\.1/g) || []).length >= 2;
  record({
    category: cat, title: "HTTP pipelining — multiple requests on one socket", severity: "low",
  ...
  const keepAliveFlood = "GET /healthz HTTP/1.1\r\nHost: t\r\nConnection: keep-alive\r\n\r\n".repeat(50);
  const t1 = Date.now();
  const kaRes = await rawSend(port, keepAliveFlood, 2000);
  record({
    category: cat, title: "Connection keep-alive flood (50 pipelined requests)", severity: "medium",
  ...
  record({
    category: cat, title: "Range header abuse (excessive range)", severity: "low",
  ...
  const lang = await http("GET", "/healthz", { headers: { "accept-language": "<script>alert(1)</script>" } });
  record({
    category: cat, title: "Accept-Language header injection (XSS vector)", severity: "low",
  ...
  record({
    category: cat, title: "Authorization header format confusion (empty token)", severity: "low",
    attack: "GET /admin with Authorization: Bearer (empty token)",
    observed: `status ${authConfusion.status}`,
    verdict: authConfusion.status === 401 || authConfusion.status === 403 ? "DEFENDED" : "VULNERABLE",
  });

  // 15. ReDoS-style WAF evasion via nested SQL comments
  const nestedComment = await http("GET", `/search?q=${encodeURIComponent("1/**/OR/**/(SELECT/**/1)")}`);
  record({
    category: cat, title: "WAF evasion — nested SQL comment obfuscation", severity: "medium",
  ...
  record({
    category: cat, title: "Mixed encoding (% + overlong UTF-8) in query", severity: "medium",
  ...
  const hostInject = await rawSend(port, `GET http://evil.com/healthz HTTP/1.1\r\nHost: ${HOST}:${port}\r\n\r\n`);
  record({
    category: cat, title: "Host header injection via absolute URI", severity: "medium",
  ...
  const doubleCT = await rawSend(port, "POST /items HTTP/1.1\r\nHost: t\r\nContent-Type: text/plain\r\nContent-Type: application/json\r\n\r\n{\"name\":\"x\",\"price\":1}");
  record({
    category: cat, title: "Double Content-Type header confusion", severity: "medium",
  ...
  const noTrustProxy = await http("GET", "/healthz", { headers: { "x-forwarded-for": "1.2.3.4", "x-real-ip": "5.6.7.8" } });
  record({
    category: cat, title: "X-Forwarded-For spoofing on non-trusted route", severity: "low",
    attack: "GET /healthz with X-Forwarded-For: 1.2.3.4 (app has trustProxy: true, so this is informational)",
  ...
  record({
    category: cat, title: "JSON large numeric overflow (price field)", severity: "medium",
  ...
  record({
    category: cat, title: "JSON with JavaScript-style comments", severity: "medium",
  ...
  record({
    category: cat, title: "Unicode control character (NUL) in JSON key", severity: "medium",
  ...
  record({
    category: cat, title: "JSON nested object depth (60 levels)", severity: "medium",
    attack: "POST /sink with 60-level nested JSON object",
    observed: `status ${nestedRes.status} in ${Date.now() - t2}ms`,
    verdict: (nestedRes.status === 200 || nestedRes.status === 400) && Date.now() - t2 < 2000 ? "DEFENDED" : "VULNERABLE",
  });

  // 24. Header name case folding abuse
  const caseFold = await rawSend(port, "GET /healthz HTTP/1.1\r\nhost: t\r\nHOST: evil\r\nHoSt: other\r\n\r\n");
  record({
    category: cat, title: "Header name case-folding abuse (duplicate Host via case variation)", severity: "medium",
  ...
  record({
    category: cat, title: "URL-encoded NUL byte in path (null injection)", severity: "medium",
  ...
  const dots = await rawSend(port, "GET /" + "../".repeat(100) + "healthz HTTP/1.1\r\nHost: t\r\n\r\n");
  record({
    category: cat, title: "Excessive dot-segments in path (path traversal stress)", severity: "medium",
    attack: "GET with 100x '../' prefix before healthz",
    observed: `response: ${dots.statusLine || "(dropped)"}`,
    verdict: dots.status === 400 || dots.status === 404 || dots.status === 200 ? "DEFENDED" : "INFO",
  });

  // 27. POST with empty Content-Length
  const emptyCL = await rawSend(port, "POST /items HTTP/1.1\r\nHost: t\r\nContent-Type: application/json\r\nContent-Length: 0\r\n\r\n");
  record({
    category: cat, title: "POST with empty body (Content-Length: 0)", severity: "low",
  ...
  const teObf = await rawSend(port, "POST /items HTTP/1.1\r\nHost: t\r\nTransfer-Encoding:\tchunked\r\nContent-Length: 4\r\n\r\n0\r\n\r\n");
  record({
    category: cat, title: "Transfer-Encoding obfuscation (tab separator)", severity: "high",
  ...
  const kidTraverse = forgeJwt({ alg: "HS256", typ: "JWT", kid: "../../../etc/passwd" }, { sub: "alice", scopes: ["admin"], exp: Math.floor(Date.now() / 1000) + 600 });
  const kidRes = await http("GET", "/admin", { headers: { authorization: `Bearer ${kidTraverse}` } });
  record({
    category: cat, title: "JWT kid header directory traversal", severity: "medium",
  ...
  const rate1 = await http("POST", "/login", { headers: { "content-type": "application/json" }, body: JSON.stringify({ user: "alice", pass: "wrong1" }) });
  const rate2 = await http("POST", "/login", { headers: { "content-type": "application/json" }, body: JSON.stringify({ user: "alice", pass: "wrong2" }) });
  record({
    category: cat, title: "Rate-limit response consistency (enumeration probe)", severity: "low",
  ...
  record({
    category: cat, title: "Missing Host header", severity: "medium",
  ...
  const t3 = Date.now();
  const limitRes = await http("POST", "/wide", { headers: { "content-type": "application/json" }, body: JSON.stringify(atLimit) });
  record({
    category: cat, title: "JSON key-count boundary (exactly 10,000 keys = jsonMaxKeys default)", severity: "medium",
    attack: "POST /wide with exactly 10,000 keys",
    observed: `status ${limitRes.status} in ${Date.now() - t3}ms`,
    verdict: (limitRes.status === 200 || limitRes.status === 400) && Date.now() - t3 < 3000 ? "DEFENDED" : "VULNERABLE",
  });
  ...
  record({
    category: cat, title: "Access-Control-Expose-Headers leakage", severity: "low",
  ...
  const wsProto = await rawSend(port, [
    "GET /ws HTTP/1.1",
    `Host: 127.0.0.1:${port}`,
    "Upgrade: websocket",
    "Connection: Upgrade",
  ...
  record({
    category: cat, title: "WebSocket subprotocol smuggling", severity: "low",
  ...
  const teNul = await rawSend(port, "POST /items HTTP/1.1\r\nHost: t\r\nTransfer-Encoding: chun\x00ked\r\nContent-Length: 4\r\n\r\n0\r\n\r\n");
  record({
    category: cat, title: "Transfer-Encoding with NUL byte in value", severity: "high",
  ...
  record({
    category: cat, title: "Very long query string (50k chars)", severity: "medium",
    attack: "GET /search?q=AAAA... (50,000 chars)",
    observed: `status ${lqRes.status} in ${Date.now() - t4}ms`,
    verdict: (lqRes.status === 200 || lqRes.status === 400 || lqRes.status === 414 || lqRes.status === 431) && Date.now() - t4 < 2000 ? "DEFENDED" : "VULNERABLE",
  });

  // 37. JSON array with massive number of elements
  const bigArray = JSON.stringify(new Array(100_000).fill("x"));
  const t5 = Date.now();
  const baRes = await http("POST", "/sink", { headers: { "content-type": "application/json" }, body: `{"data":${bigArray}}` });
  record({
    category: cat, title: "JSON massive array (100k elements)", severity: "medium",
    attack: "POST /sink with 100,000-element array",
    observed: `status ${baRes.status} in ${Date.now() - t5}ms`,
    verdict: (baRes.status === 200 || baRes.status === 400) && Date.now() - t5 < 3000 ? "DEFENDED" : "VULNERABLE",
  });
  ...
  record({
    category: cat, title: "Dual credential confusion (Bearer + Cookie)", severity: "low",
  ...
  record({
    category: cat, title: "HTTP/1.0 request (no Host header)", severity: "low",
  ...
  const clChunked = await rawSend(port, "POST /items HTTP/1.1\r\nHost: t\r\nContent-Type: application/json\r\nContent-Length: 10\r\n\r\n5\r\n{\"a\":1}\r\n0\r\n\r\n");
  record({
    category: cat, title: "Content-Length + chunked body mismatch", severity: "medium",
...
async function main() {
  ...
  await new Promise<void>((resolve, reject) => {
    const timer = setTimeout(() => reject(new Error("target did not announce ready in 15s")), 15_000);
    ...
    target.on("exit", (code) => { if (code !== 0 && code !== null) reject(new Error(`target exited ${code}`)); });
  });
  ...
  const criticalHigh = findings.filter((f) => (f.severity === "critical" || f.severity === "high") && f.verdict === "VULNERABLE");
  console.log("\n" + "=".repeat(80));
