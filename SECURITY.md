...
| **(1)(i) Minimize impact on other networks**                            | `fetchGuard()` default-denies loopback, RFC1918, link-local (cloud metadata IPs), unique-local, CGNAT, Oracle `192.0.0.0/24`, IANA-reserved, multicast, and non-http(s) schemes. Manual redirect follow re-validates each hop; the IPv4 address embedded in IPv4-mapped, SIIT, IPv4-compatible, NAT64 (`64:ff9b::/96`, `64:ff9b:1::/48`), 6to4 and Teredo IPv6 addresses is re-checked against the IPv4 policy.                                                                                                                        |
