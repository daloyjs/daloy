...
export default function Page() {
  return (
    <>
      ...
      <p>
        The{" "}
        <a href="https://ai-sdk.dev/" target="_blank" rel="noreferrer noopener">
          Vercel AI SDK
        </a>{" "}
      ...
      <p>
        ...
        behind <code>fetchGuard()</code>
        {", "}and the secure-by-default layer that wraps all of it.
      </p>
      ...
      <p>
        ...
        API before it ever reaches the model, and the model&apos;s output
        streams back through DaloyJS unbuffered.
      </p>
      <FlowDiagram
      ...
      <p>
        The AI SDK&apos;s <code>result.toUIMessageStreamResponse()</code>{" "}
        returns a web-standard <code>Response</code>
        {", "}and a DaloyJS handler can return a raw <code>Response</code>{" "}
        directly after explicitly setting{" "}
        <code>acknowledgeNoResponseBodySchema: true</code>
        {". "}Without that acknowledgement, the route fails closed instead of
        silently bypassing response validation. The acknowledged stream passes
        through, backpressure and all, and the framework still finalizes it: it
        adds the request id, applies <code>secureHeaders()</code> and CORS, runs
        your <code>onSend</code> hooks, and strips fingerprint headers, exactly
        as it does for a structured result. This endpoint works with the AI
        SDK&apos;s <code>useChat()</code> hook on the client with no extra
        wiring.
      </p>
      <CodeBlock language="ts" code={CHAT} />

      <h2 id="structured-output-validated-by-your-contract">
      ...
      <h2 id="tool-calling-behind-fetchguard">
        Tool calling behind fetchGuard
      </h2>
      <p>
        AI SDK 7&apos;s tool loop is where prompt injection becomes a
        server-side request forgery problem: the model asks a tool to fetch a
        URL, and a poisoned prompt aims it at <code>169.254.169.254</code> or
        your internal network. Run every tool fetch through{" "}
      ...
      <h2 id="what-about-openapi-and-the-typed-client">
      ...
      <ul>
        ...
        <li>
          <a href="/docs/security/fetch-guard">fetchGuard (SSRF)</a> for the
          full default-deny list and redirect handling.
        </li>
        <li>
