...
export async function GET() {
  ...
  const lines: string[] = [
    ...
    "",
    "Every public page also negotiates Markdown on the same URL: send `Accept: text/markdown` (or append `.md`). Responses set `Vary: Accept`. Errors from the HTTP APIs are RFC 9457 problem+json with `code` and `hint` fields; see GET /api and GET /openapi.json.",
    "",
    ...
    `- [DaloyJS OpenAPI spec](${SITE_URL}/docs/openapi.md): generate OpenAPI 3.1 from routes. Machine-readable website API catalog: ${SITE_URL}/openapi.json.`,
