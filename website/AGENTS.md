...
- [.agents/skills/next-cache-components-adoption/SKILL.md](.agents/skills/next-cache-components-adoption/SKILL.md) — adopt or continue Next.js Cache Components (`cacheComponents`, `instant = false`, blocking-prerender fixes). The flag is already on; remaining work is removing per-route opt-outs. Do not drop the root-layout nonce CSP (`headers()` in `app/layout.tsx`) to make a route instant.
