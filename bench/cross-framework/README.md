...
| `body-size-sweep.mjs`  | POST throughput across body sizes {100 B, 1 KiB, 16 KiB, 256 KiB, 1 MiB, 4 MiB}. Reports request **upload** MiB/s (req/s × body size); autocannon's response-side throughput (the tiny ack) is recorded separately as `responseMBps`.                                                                                                                                                                                                                                                                                                                                                                               |
...
| `streaming.mjs`        | Large `ReadableStream` response throughput in MiB/s and req/s. Preflights each fixture: the row is failed unless `GET /stream` actually delivers the full 10 MiB payload.                                                                                                                                                                                                                                                                                                                                                                                                                                           |
